...
router.delete('/deleteuser/:userId', passport.authenticate('jwt', {
  session: false
}), (req, res, next) => {
...
router.put('/edituser', passport.authenticate('jwt', {
  session: false  
}), (req, res, next) => {
  let newUser = {    
    name: req.body.name,
  ...
  let oldUserID = req.body.userId;

  User.editUser(oldUserID, newUser, (err, user) => {
    if (err) {
      res.json({
        success: false,
        msg: 'Failed to update  user'
...
router.get('/manageuser', passport.authenticate('jwt', {
  session: false
}), (req, res, next) => {
  User.find(function (err, user) {
    // if there is an error retrieving, send the error. nothing after res.send(err) will execute
